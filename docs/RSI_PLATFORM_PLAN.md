...
| --- | --- |
| A T0 mission runs end to end on real tools, produces an evidence bundle, and a person approves at the defined points | **Met.** Mission `3d255084` ran on the dev database with the local model and the Docker-sandboxed lint, sim, formal, and physical adapters: 16 model calls, 36,067 tokens, $0. Its owner approved at four points: the launch, the RTL and formal reviews (`review.tool_failures`, the known ECC-status bug), and the bundle. Bundle `bd72fec9…` is `hold`, class `planned`. All four tool runs reproduced their Phase 1 records exactly: same spec hashes and identical outputs. Every run reconstructs from the audit log, and the chain verifies (420 rows). An earlier dry run on the test database found and fixed four defects before this run |
